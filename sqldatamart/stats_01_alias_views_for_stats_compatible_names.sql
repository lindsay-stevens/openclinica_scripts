...
FROM (
       SELECT
         ($$CREATE VIEW $$
          ...
          || array_to_string(array_agg(
                                 case_constructors_trim_label_cols.case_constructors_trimmed),
                             ', ')
       ...
       FROM (
              SELECT
                lower(
                    regexp_replace(regexp_replace(case_constructors.study_name,
                                                  $$[^\w\s]$$, '', 'g'),
                                   $$[\s]$$, '_', 'g'))
                || $$.av_$$
                     ...
                     SELECT DISTINCT
                       ...
                       (
                         ...
                         || lower(regexp_replace(
                                      regexp_replace(quote_literal(study_name),
                                                     $$[^\w\s]$$, '', 'g'),
                                      $$[\s]$$, '_', 'g'))
                         || $$.$$
                         || item_group_oid
...
-- this might happen due to prepending 'i_' for items starting with an int,
                              CASE WHEN length(item_name_hint) >=
                                        64 THEN substr(item_name_hint, 1, 63)
                            ...
                            FROM (
                                   SELECT DISTINCT
                                     ...
                                     lower(
                                         item_oid)                   AS item_oid,
                                     lower(substr(item_name, 1,
                                                  32))               AS item_name
-- if the item_name starts with an integer, prepend 'i_',
                                     (CASE WHEN item_name ~ '^[0-9]+$' THEN
                                       $$i_$$ || lower(substr(item_name, 1, 12))
                                      ...
                                      || substr(
                                         regexp_replace(
                                             regexp_replace(
                                                 ...
                                                 -- remove underscore
                                                 , $$[\s]+$$, '_', 'g')
                                             -- change space(s) to underscore
                                             , $$[^\w]$$, '', 'g')
                                         -- remove non alphanum non underscore
                                         , 1, 45)
                                     )                               AS item_name_hint,
                                     item_data_type
-- use max since item_form_order may change between crf versions,
                                     max(
                                         item_form_order)            AS item_form_order,
                                     item_response_set_label
                                   FROM dm.metadata
